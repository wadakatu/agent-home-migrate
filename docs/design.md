...
実データの移行ではGitHub release tagを固定してインストールし、旧PCのexportと新PCの
restoreで同じ`ahm --version`を使う。bundle manifestの`created_by.version`は、
そのversionに対応するrelease tagからインストールした場合に生成元実装を一意に特定できる。

開発中のmain branchやtagなしcheckoutはcommitによって内容が変わるため、再現可能な
実移行元として扱わない。package metadataのversionは
`agent_home_migrate.__version__`からbuild時に生成し、CLI・bundle manifest・distribution
metadataのversionを一致させる。CIはインストール済みdistribution metadataとの一致を検証する。
